...
compWT = function(df, rs.path, colname, fps, order = 6, suffix = "", 
  ...
  dt = 1 / fps
  pad = TRUE # padding with zeros to length of power of 2, faster and less cone of influence problems
  Dj = 1 / 12 # spacing / resolution between scales, 12 sub-octaves for fine frequency resolution
  ...
  for (i in 1:nos) {
    ...
    # extract time into a vector
    time = df.sel |> pull(Frame)/fps
    
    # configure number of frequency scales (minus 1)
    J1 = round(log2(((length(time) * 0.17) * 2 * dt) / S0) / Dj)
    
    # compute the wavelet transformation
    wt = biwavelet::wt(cbind(time, data), 
                       pad = pad, dj = Dj, s0 = S0, J1 = J1, 
                       mother = "morlet", param = order)
