...
* `Recording_Participant_P1-cut.TextGrid` : containing syllables and speaking versus silent
* `Recording_Participant_P1-cut.TextGrid` : same for other interaction partner
...
kable(df.period |>
  mutate(minTime = as.numeric(minTime - startExp, units = "secs"), 
         maxTime = as.numeric(maxTime - startExp, units = "secs")) |> 
  rowwise() |>
  # Generate a sequence of start times for each increment
  mutate(winStart = list(seq(406, 427 - x, by = x))) |>
  tidyr::unnest(winStart) |>
  filter((minTime >= winStart & minTime <= (winStart + 5)) | 
           ((maxTime >= winStart) &  maxTime <= (winStart + 5))) |>
  ungroup() |>
  group_by(Identifier, winStart) |> 
  summarise(value = paste(unique(AOI), collapse = "; "),
            .groups = "drop") |>
  tidyr::pivot_wider(names_from = Identifier) |>
  mutate(Increment = sprintf("%.0fmin %.0fsec to %.0fmin %.0fsec",
                             floor(winStart/60), winStart%%60,
                             floor((winStart+x)/60), (winStart+x)%%60)) |>
    select(Increment, P1, P2))
